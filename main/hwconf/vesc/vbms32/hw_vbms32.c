...
static esp_err_t i2c_select_device(uint8_t address) {
	if (!i2c_device) {
		i2c_device_config_t config = {.dev_addr_length = I2C_ADDR_BIT_LEN_7, .device_address = address, .scl_speed_hz = I2C_SPEED};
...
static esp_err_t i2c_create_bus(void) {
	i2c_master_bus_config_t config = {.i2c_port = I2C_NUM_0, .sda_io_num = PIN_SDA, .scl_io_num = PIN_SCL, .clk_source = I2C_CLK_SRC_DEFAULT, .glitch_ignore_cnt = 7, .flags.enable_internal_pullup = true};
...
static lbm_value ext_bms_init(lbm_value *args, lbm_uint argn) {
	...
	if (i2c_device) { i2c_master_bus_rm_device(i2c_device); i2c_device = NULL; }
