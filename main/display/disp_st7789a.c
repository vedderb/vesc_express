...
static uint16_t to_disp_color(uint32_t rgb) {
	...
	uint8_t color_high = r << 3;
	color_high |= (g >> 3);

	uint8_t color_low = g << 5;
	color_low |= b;
...
bool disp_st7789a_render_image(image_buffer_t *img, uint16_t x, uint16_t y, color_t *colors) {
	uint16_t cs = x;
	...
	disp_st7789a_command(0x2A, col, 4);
	disp_st7789a_command(0x2B, row, 4);
...
void disp_st7789a_clear(uint32_t color) {
...
static lbm_value ext_disp_cmd(lbm_value *args, lbm_uint argn) {
	...
	if (argn > 1) {
		...
		disp_st7789a_command(cmd, paras, argn - 1);
		res = ENC_SYM_TRUE;
	} else if (argn == 1) {
		uint8_t cmd = (uint8_t)lbm_dec_as_u32(args[0]);
		disp_st7789a_command(cmd, 0, 0);
		res = ENC_SYM_TRUE;
	}
...
static lbm_value ext_disp_orientation(lbm_value *args, lbm_uint argn) {
	...
	case 0:
		arg = 0x00;
		disp_st7789a_command(0x36, &arg, 1);
		display_width  = 240;
		display_height = 320;
		break;
	case 1:
...
void disp_st7789a_init(int pin_sd0, int pin_clk, int pin_cs, int pin_reset, int pin_dc, int clock_mhz) {
	// SPI Mode 3 (CPOL=1, CPHA=1) required by ST7789A
	hwspi_init(clock_mhz, 3, -1, pin_sd0, pin_clk, pin_cs);
...
static const uint8_t init_cmds[][16] = {
		{2, 0x36, 0x60}, // Memory Data Access Control (landscape, RGB)
		{2, 0x3A, 0x55}, // Interface Pixel Format: 16bpp RGB565
		{6, 0xB2, 0x0C, 0x0C, 0x00, 0x33, 0x33}, // Porch Setting
		{2, 0xB7, 0x35}, // Gate Control
		{2, 0xBB, 0x19}, // VCOM Setting (ST7789A default)
		{2, 0xC0, 0x2C}, // LCM Control
		{2, 0xC2, 0x01}, // VDV and VRH Command Enable
		{2, 0xC3, 0x12}, // VRH Set
		{2, 0xC4, 0x20}, // VDV Set
		{2, 0xC6, 0x0F}, // Frame Rate Control: 60 Hz
		{3, 0xD0, 0xA4, 0xA1}, // Power Control 1
		{15, 0xE0, 0xD0, 0x04, 0x0D, 0x11, 0x13, 0x2B, 0x3F, 0x54, 0x4C, 0x18, 0x0D, 0x0B, 0x1F, 0x23}, // Positive Gamma
		{15, 0xE1, 0xD0, 0x04, 0x0C, 0x11, 0x13, 0x2C, 0x3F, 0x44, 0x51, 0x2F, 0x1F, 0x1F, 0x20, 0x23}, // Negative Gamma
		{1, 0x21}, // Display Inversion On
};

void disp_st7789a_reset(void) {
	if (m_pin_reset >= 0) {
	...
	disp_st7789a_command(0x11, 0, 0); // Sleep Out
	vTaskDelay(120);
	...
	for (int i = 0; i < 14; i++) {
		...
		disp_st7789a_command(cmd, args, argn);
	}

	disp_st7789a_command(0x29, 0, 0); // Display On
	vTaskDelay(120);

	display_width  = 320;
	display_height = 240;

	disp_st7789a_clear(0);
}
